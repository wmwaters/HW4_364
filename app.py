...
# William Waters
# asked questions to Austin McCall
...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:19900241@localhost/wmwatershw4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
class User(UserMixin, db.Model):
    ...
    users_collection = db.relationship('PersonalGifCollection', backref = 'User')
...
def get_gifs_from_giphy(search_string):
    ...
    fullurl = baseurl + '?apikey=' + api_key + '&q=' + str(search_string) + '&limit=5'
    json_file = json.loads(requests.get(fullurl).text)
...
def get_or_create_gif(title, url):
    ...
    first = Gif.query.filter_by(title = title).first()
...
def get_or_create_search_term(term):
    ...
    first = SearchTerm.query.filter_by(term = term).first()
    if not first: first = SearchTerm(term = term)
    gifs = get_gifs_from_giphy(term)
    for gif in gifs:
        title = gif['title']
        url = gif['embed_url']
        gif = get_or_create_gif(title, url)
        first.gifs.append(gif)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    first = PersonalGifCollection.query.filter_by(name = name, userid = current_user.id).first()
    if first: return first
    first = PersonalGifCollection(name = name, userid = current_user.id)
    for gif in gif_list:
        first.gifs.append(gif)
    db.session.add(first)
    db.session.commit()
...
def create_collection():
    ...
    if form.validate_on_submit():
        ...
        for pick in form.gif_picks.data:
            gifs.append(get_gif_by_id(int(pick)))
        c = get_or_create_collection(form.name.data, current_user, gifs)
